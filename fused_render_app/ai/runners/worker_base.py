...
`fused_render_app.ai.supervisor` on the interpreter built from that declaration.
...
#: `fused_render_app` package on `sys.path` (see its own top-of-file note), so a
...
#: `fused_render_app` is not importable.
