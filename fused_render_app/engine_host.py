...
`_validate`, reinit replay stays as a no-op registry) — Render App only has
...
def _spawn_env(child: Child) -> dict:
    ...
    if child.python != sys.executable:
        from fused_render_app.env import _STRIPPED_ENV_VARS
...
def _validate_background(engine_id: str, python: str, daemon: str,
    ...
    from fused_render_app import background_apps
