...
from fused_render_app.ai import catalog, fit, footprints, hw_detect, registry, supervisor
...
from fused_render_app.ai.runners import diarize, embed_common, engine_options, formats, partial, preview
from fused_render_app.routes.common import (
    ...
    **`repo` puts that same translation ON THE WIRE, because a client cannot
    ...
    for row in rows:
        ...
        for entry in row["models"]:
            ...
            entry["speedEstimate"] = None  # no benchmarking in fused-render-app
...
#: `_machine_ram_gb` and `_fit_verdict` moved to `fused_render_app/ai/fit.py`
...
def _apple_catalog() -> dict:
    ...
    from fused_render_app.ai.apple import host as apple_host
...
def _apple_transcribe(body: dict, source: str, task: str, diarizing: bool, wants_words: bool):
    ...
    """
    from fused_render_app.ai.apple import host as apple_host
    from fused_render_app.ai.apple import speech as apple_speech
