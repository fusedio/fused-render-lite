...
No import of anything under `fused_render_app.server` — the router imports this
...
# A scheduled message's own row, by id (`fused_render_app/schedule.py`'s
...
def list_jobs(*, now: float | None = None, mark_read: bool = False) -> list[dict]:
    ...
    **`mark_read` defaults to False, and that default is deliberate, not an
    ...
    /api/jobs` — `grep -rn "list_jobs(" fused_render_app/` also finds
