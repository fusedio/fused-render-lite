...
# "Available Python libraries") — the `[bundled]` extra plus `pyarrow`,
# `duckdb` and `httpx`, which fused-render only had because they are core
# server dependencies there — minus the cloud credential chains (botocore,
# google-auth) and the fused engine (+ mcp). Those apps were written against
# this implicit set, so Render App installs it once, on demand, into one
# shared venv. Keep this tuple and the skill's list in step: an import that
# works in fused-render but fails here is exactly the bug this exists to avoid.
LEGACY_DEPS = (
    "numpy",
    "pandas",
    "pyarrow>=14",
...
def legacy_project_dir() -> str:
    ...
    text = ("# Generated by fused-render-app: the implicit environment for .fused apps\n"
            ...
            "# pyarrow/duckdb/httpx, minus cloud credential chains and the fused\n"
            "# engine). Do not edit.\n"
