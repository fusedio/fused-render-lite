...
# The environment an app WITHOUT a pyproject.toml gets: what fused-render's
# authoring skill promises such an app (skills/fused-render-authoring/SKILL.md,
# "Available Python libraries") — the `[bundled]` extra plus `httpx`, which
# fused-render only had because it is a core server dependency there — minus
# the cloud credential chains (botocore, google-auth), the fused engine
# (+ mcp), and `pyarrow` / `duckdb` (122 MB + 44 MB installed, deliberately
# dropped; a legacy app importing either fails here). Those apps were written
# against this implicit set, so Render App installs it once, on demand, into
# one shared venv. Keep this tuple and the skill's list in step apart from the
# pyarrow/duckdb exception.
LEGACY_DEPS = (
    "numpy",
    "pandas",
    "requests",
...
def legacy_project_dir() -> str:
    ...
    text = ("# Generated by fused-render-app: the implicit environment for .fused apps\n"
            "# that ship no pyproject.toml (fused-render's implicit set: [bundled] plus\n"
            "# httpx, minus pyarrow, duckdb, cloud credential chains and the fused\n"
            "# engine). Do not edit.\n"
