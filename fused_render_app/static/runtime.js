/*
 * fused-render-app runtime. Injected into every rendered page (/render).
 *
...
(function () {
  ...
  // that needs a capability Render App does not have.
  function unsupported(name) {
    const err = new Error(name + " is not supported on Render App");
    err.type = "unsupported";
    console.error("[render-app] " + err.message);
    return err;
  }
  ...
  // ?path= instead of multipart) because Render App's server has no form parser.
  ...
  // file watcher Render App does not have, and the app should know: throw.
  ...
  // `_preview=1` on its /render URL. Render App renders no thumbnails today, so
  ...
  function daemonRun(params) {
    ...
    return ready.then(() => {
      ...
      // Render App has no live reload, so nothing to watch here.)
      if (!data.ok) {
  ...
  window.fused = {
    ...
    // Not supported in Render App: every one of these throws when touched.
    uploadFile,
    mkdir,
    trackJob,
    watchJob,
