...
relative to the `fused_render_lite` package, because the app's own path is not stable —
...
#: The installed `fused_render_lite` package directory, and the one prefix
...
def _venv_identity(project_dir: str) -> str:
    ...
    Relativising against the package makes the identity `<fused_render_lite>/ai/runners/
    ...
    corner: any two copies of `fused_render_lite` on one machine share these keys, since
...
def _probe_writable(path: str) -> bool:
    ...
    import `fused_render_lite` (D152) — two copies of the technique is correct here.
...
def _use_home_store(project_dir: str) -> bool:
    ...
    2. The folder ships inside the installed `fused_render_lite` package (an AI
...
def _template_roots() -> list[str]:
    ...
    """
    # fused-render-lite ships no preview templates; the user-template root is
...
def project_root_for(path: str) -> str | None:
    ...
    runs in".
    """
    def app_dir_for(_p: str):  # no ~/Fused workspace in lite: every folder is its own root
        return None
...
def write_sidecar(venv_dir: str, project_dir: str, digest: str) -> None:
    ...
    `<fused_render_lite>/ai/runners/…` for one that ships inside the app. Recording the
...
def _sidecar_source_dir(source: str) -> str:
    ...
    The inverse of `_venv_identity` for the in-app case: `<fused_render_lite>/ai/runners/
