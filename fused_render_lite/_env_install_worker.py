...
must stay free of any `fused_render_lite` import (D152 — importing the package in a
...
`fused_render_lite/templates/docs/install_worker.py` writes for the typst download,
...
Stdlib only: no `fused_render_lite` import, and (since the switch to `uv sync`) no
...
# Stages and their percentages, kept in step with fused_render_lite/envinstall.py's
...
# `import fused_render_lite` in a detached child is exactly the bootstrap that broke
...
# Kept in step with fused_render_lite/envinstall.READY_MARKER and
...
#: `fused_render_lite` at all (D152 — a detached child that bootstraps the package is
...
def _write(progress_dir, stage, pct, detail="", done=False, error=None,
    ...
    # its current meaning and format (`fused_render_lite/engine.py` and
...
class _UvProgress:
    ...
    **Locked, because two threads genuinely touch this concurrently**:
    ...
    path. `fused_render_lite.envinstall._remember_ending` documents the same
    ...
    **The announced total is a LOWER BOUND that can only grow**, never shrink
    ...
    def snapshot(self, elapsed):
        ...
        which is the contract `_ensure_venv` (fused_render_lite/ai/supervisor.py)
...
def _venv_runs(venv_dir):
    ...
    must not import `fused_render_lite` (D152) — with the SAME three-valued answer,
...
def _state_digest(project_dir):
    ...
    than imported because this file must stay free of any `fused_render_lite` import
    (D152).
    """
    try:
...
#: The `fused_render_lite` package directory. This file LIVES in the package — it is
#: `fused_render_lite/_env_install_worker.py`, spawned by path — so its own dirname is
...
_PACKAGE_IDENTITY = "<fused_render_lite>"
...
#: this file (D152: no `fused_render_lite` import in a detached child). That module
...
def _sync_root(project_dir, venv_dir):
    ...
    That is not a hypothetical: the AI runner folders (`fused_render_lite/ai/runners/*`)
