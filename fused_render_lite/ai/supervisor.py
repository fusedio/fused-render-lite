...
from fused_render_lite import jobs
from fused_render_lite._view_url_codec import canonical_fs_path
from fused_render_lite.ai import catalog, fit, footprints, hub_metadata, hw_detect, registry
...
def _mirror_ok(model: str) -> str:
    ...
    `mirror` as bare modules with no `fused_render_lite` package on `sys.path`. But
    ...
    if not model:
        return ""
    try:
        from fused_render_lite.ai import catalog
...
def _child_env(token: str, model: str = "", capability: str = "") -> dict:
    ...
    **`FUSED_AI_MEMORY_BUDGET_BYTES` carries `fit.available_budget_bytes()`
    ...
    `fused_render_lite.ai.fit`/`hw_detect` (see `formats.py`'s own top-of-file
...
def _require_build_tools() -> None:
    ...
    """
    from fused_render_lite import env as _lite_env

    if not _lite_env.uv_bin(download=True):
        raise SupervisorError("uv is not available, so the model environment cannot be built")
    from fused_render_lite import engine
...
def _ensure_venv(runner: registry.Runner, worker: Worker, job: str) -> str:
    ...
    """
    from fused_render_lite import envinstall, projectenv
...
def _fetch_only(runner: registry.Runner, model: str, job: str) -> None:
    ...
    try:
        ...
        from fused_render_lite.ai import hub_cache
