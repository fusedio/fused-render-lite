...
**Stdlib only, and no import of `fused_render_lite`.** It is imported by every
...
`fused_render_lite.ai.runners.formats`. Both readings must work, so nothing here may
...
def select_gguf_recipe(files: dict[str, int | None], budget_bytes: float | None,
    ...
    **A PARTIALLY-known shard set** (some parts sized, some not -- code
    ...
    module's own top-of-file note) never pays for `fused_render_lite.ai.fit`'s
    ...
    for token, names in groups.items():
        ...
        if known:
            ...
            from fused_render_lite.ai import fit  # lazy: see docstring above
