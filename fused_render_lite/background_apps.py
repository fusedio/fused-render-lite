...
only lite changes are the import swaps noted inline (see engine_host.py's
...
from fused_render_lite import env, paths
...
def load_manifest(folder: str) -> Manifest | None:
    ...
    import tomllib  # lite pins Python 3.12: stdlib tomllib is always there
...
def version_for(folder: str, interpreter: str) -> str:
    ...
    if manifest.main:
        ...
        from fused_render_lite import engine_host
...
def interpreter_for(folder: str) -> str:
    ...
    NOT `projectenv.interpreter_for` (also present in lite, copied in with
...
def cache_dir_for(engine_id: str) -> str:
    ...
    `apps/`: in lite `paths.apps_dir()` holds the EXTRACTED .fused payloads
