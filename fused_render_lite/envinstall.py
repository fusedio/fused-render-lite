...
def _backend_attr(name: str):
    ...
    there is no guess.
    """
    from fused_render_lite.engine import get_backend
...
def _venv_runs(venv_dir: str) -> bool | None:
    ...
    try:
        from fused_render_lite.engine import _child_env
...
def uv_bin() -> str | None:
    """The uv binary (lite: `env.uv_bin`, which also downloads it on first use)."""
...
def _spawn(key: str, project_dir: str, acquire_python: str | None = None,
    ...
    **That split is the whole point, and it is D277's crash one layer down.**
    ...
    environment (`fused_render_lite.ai.supervisor._ensure_venv`) means a model that
    can never be loaded.
    ...
    there, because the worker must not import `fused_render_lite` (D152: importing the
...
def _mirror_into_jobs(key: str, project_dir: str, downloading_python: bool = False) -> None:
    ...
    that says so.
    """
    from fused_render_lite import jobs, projectenv
